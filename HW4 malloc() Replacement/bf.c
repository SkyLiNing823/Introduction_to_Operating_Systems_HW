...
void *malloc(size_t size)
{
    ...
    if (size % 32 != 0)
        size += (32 - (size % 32));
    struct block *Block = (void *)memory;
    size_t closestSize = 20000;
    struct block *selectBlock;
    while (Block != NULL)
    {
        if (Block->size >= size && Block->free == 1)
        {
            if (Block->size > size)
            {
                if (closestSize > Block->size)
                {
                    closestSize = Block->size;
                    selectBlock = (void *)Block;
                }
            }
            else
            {
    ...
    if (Block == NULL)
    {
        ...
        remainBlock = (void *)selectBlock + 32 + size;
        remainBlock->size = (selectBlock->size) - size - 32;
        selectBlock->size = size;
        remainBlock->free = 1;
        selectBlock->free = 0;
        remainBlock->prev = selectBlock;
        remainBlock->next = selectBlock->next;
        selectBlock->next = remainBlock;
    }
    return (void *)selectBlock + 32;
}
